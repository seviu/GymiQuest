...
describe("assessment UI flow", () => {
  ...
  it("opens a reproducible task report in a new tab and lets the learner pause the question topic", () => {
    ...
    const theorySupport = container.querySelector(`[data-topic-theory-support="${review.topicIds[0]}"]`)
    expect(theorySupport).not.toBeNull()
    expect(theorySupport?.textContent).toContain("NOCH EIN ZUGANG")
    expect(theorySupport?.textContent).toContain("Ein neues Beispiel mit Lösungsweg")
  ...
  it("also offers to skip the topic before a lesson's questions begin", () => {
    ...
    act(() => buttonWithText(container, "Ich verstehe dieses Thema noch nicht").click())
    const theorySupport = container.querySelector(`[data-topic-theory-support="${lesson.topicIds[0]}"]`)
    expect(theorySupport).not.toBeNull()
