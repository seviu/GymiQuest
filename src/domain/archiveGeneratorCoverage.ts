...
function generateRelationalSystemQuestion(
  ...
  return {
    ...
    prompt: localText(
      locale,
      `In zwei Kisten liegen zusammen ${total} Bausteine. In der blauen Kiste liegen ${multiplier}-mal so viele Bausteine wie in der gelben Kiste und zusätzlich ${offset} Bausteine. Wie viele Bausteine liegen in der ${askedGroup}?`,
      `Two boxes contain ${total} building blocks altogether. The blue box contains ${multiplier} times as many blocks as the yellow box, plus ${offset} extra blocks. How many blocks are in the ${askedGroup}?`,
      `In due scatole ci sono in tutto ${total} mattoncini. Nella scatola blu ce ne sono ${multiplier} volte quanti nella scatola gialla, più altri ${offset}. Quanti mattoncini ci sono nella ${askedGroup}?`,
      `En dos cajas hay ${total} bloques en total. En la caja azul hay ${multiplier} veces tantos bloques como en la caja amarilla, más ${offset} bloques adicionales. ¿Cuántos bloques hay en la ${askedGroup}?`,
    ),
    ...
    hint: localText(
      locale,
      `Ziehe zuerst die zusätzlichen ${offset} Bausteine von der Gesamtsumme ab. Der Rest besteht aus ${multiplier + 1} gleich grossen Teilen.`,
      `First subtract the ${offset} extra blocks from the total. The remainder consists of ${multiplier + 1} equal parts.`,
      `Sottrai prima i ${offset} mattoncini aggiuntivi dal totale. Il resto è formato da ${multiplier + 1} parti uguali.`,
      `Primero resta los ${offset} bloques adicionales del total. El resto está formado por ${multiplier + 1} partes iguales.`,
    ),
    easierExplanation: localText(
      locale,
      `Nenne die Anzahl in der gelben Kiste einen Teil. Dann enthält die blaue Kiste ${multiplier} solche Teile plus ${offset}. Zusammen sind das ${multiplier + 1} Teile plus ${offset}.`,
      `Call the number in the yellow box one part. The blue box then contains ${multiplier} such parts plus ${offset}. Altogether that is ${multiplier + 1} parts plus ${offset}.`,
      `Chiama una parte il numero nella scatola gialla. La scatola blu contiene quindi ${multiplier} parti più ${offset}. In tutto sono ${multiplier + 1} parti più ${offset}.`,
      `Llama una parte a la cantidad de la caja amarilla. La caja azul contiene entonces ${multiplier} partes más ${offset}. En total son ${multiplier + 1} partes más ${offset}.`,
    ),
    explanation: localText(
      locale,
      `${total} − ${offset} = ${total - offset}. Geteilt durch ${multiplier + 1} ergibt das ${smaller} Bausteine in der gelben Kiste. In der blauen sind ${multiplier} · ${smaller} + ${offset} = ${larger}.`,
      `${total} − ${offset} = ${total - offset}. Dividing by ${multiplier + 1} gives ${smaller} blocks in the yellow box. The blue box has ${multiplier} · ${smaller} + ${offset} = ${larger}.`,
      `${total} − ${offset} = ${total - offset}. Dividendo per ${multiplier + 1} si ottengono ${smaller} mattoncini nella scatola gialla. In quella blu ce ne sono ${multiplier} · ${smaller} + ${offset} = ${larger}.`,
      `${total} − ${offset} = ${total - offset}. Al dividir entre ${multiplier + 1} se obtienen ${smaller} bloques en la caja amarilla. En la azul hay ${multiplier} · ${smaller} + ${offset} = ${larger}.`,
    ),
    workedSteps: [
      `${total} − ${offset} = ${total - offset}`,
      `${total - offset} : ${multiplier + 1} = ${smaller}`,
      `${multiplier} · ${smaller} + ${offset} = ${larger}`,
    ],
    visual: {
