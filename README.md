...
- Deterministic dynamic generators: every task has fresh values, while its seed allows the exact question to be reproduced for grading and debugging. Generation v6 adds relational systems, voxel-solid counting, recurring cycles, number walls, and fraction number lines through ten archive-informed templates and 6,152 solved template/configuration pairs. Persisted generation v2–v5 tasks and unversioned legacy tasks retain their historical routing for exact paused-session replay. The topic-specific ranker uses structure such as multi-step rates, complements, rotations, perimeter traps, constructions, and spatial moves—not a cosmetic label or larger-number shortcut.
...
- The private German source shelf registers all 49 supplied 2015–2026 PDFs by exact SHA-256 and page count: twelve language papers, solutions, text sheets, and essay-prompt sets plus the separate 2015 essay-correction guidance. One multi-file picker identifies renamed documents by content, while a compact year selector avoids presenting twelve dense cards at once. Each year can launch either a persisted 45-minute language-paper workspace, where the text sheet remains available and solutions unlock only after irreversible submission or timeout, or a persisted 60-minute source-essay workspace with a side-by-side prompt PDF, local title/draft autosave, live word count, and bounded self-check. These source practices deliberately create no points, XP, grade, mastery, or adaptive evidence.
...
- A scheduled review remains worth its full smaller XP award even when it exposes mistakes or needs help. Reviews never use the Aufbau band: fragile memories receive full Standard retrieval, while stable later reviews become Prüfungsnah. The outcome changes retention and the next due date, never the fixed review XP. No mastered topic can disappear for more than 21 days after its latest recorded practice: an overdue-by-inactivity refresh moves to the front with new values.
- A learner who struggles with a lesson is not immediately sent through the same idea again. Automatic recommendations leave an 8-hour break after a light miss and a 24-hour break after repeated mistakes or hint use, offer other ready work meanwhile, then return with a fresh deterministic seed and gentler difficulty when needed.
...
src/domain/archiveGeneratorExpansion.ts generation-v5 archive-informed families
src/domain/archiveGeneratorCoverage.ts generation-v6 archive-coverage families and diagnostics
...
src/domain/authorValidation.ts zero-XP production-generator samples and 86-field validation coverage
...
docs/2015-2026-german-recurrence-matrix.md German source registry, task recurrence, and scoring boundary
