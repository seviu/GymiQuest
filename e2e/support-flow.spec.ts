...
test("reports a generated task, pauses its topic, shows an honest mock trend, reopens it, and resets onboarding", async ({ page, context }) => {
  ...
  await expect(alternateTheory).toBeVisible()
  await expect(alternateTheory.getByRole("heading", { name: /Schauen wir .+ anders an/u })).toBeVisible()
  const alternateTheoryAudit = await new AxeBuilder({ page })
    .include("[data-topic-theory-support]")
    .withTags(wcagTags)
    .analyze()
  expect(alternateTheoryAudit.violations).toEqual([])
  await expect(page.getByRole("alert")).toHaveCount(0)
  await alternateTheory.getByRole("button", { name: "Jetzt ist es klarer" }).click()
  await expect(alternateTheory).toHaveCount(0)
  ...
  await expect(page.locator('[aria-label="0 von 86 Prüffeldern in dieser Sitzung geprüft"]')).toBeVisible()
  ...
  await page.getByRole("button", { name: "Als geprüft markieren", exact: true }).click()
  await expect(page.locator('[aria-label="1 von 86 Prüffeldern in dieser Sitzung geprüft"]')).toBeVisible()
